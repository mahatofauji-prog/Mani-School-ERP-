...
export default function Schools() {
  ...
  const createMut = useMutation({
    mutationFn: async (payload) => (await api.post("/schools", payload)).data,
    onSuccess: () => { toast.success("School created with School Admin login"); qc.invalidateQueries({ queryKey: ["schools"] }); setOpen(false); setForm(empty); },
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <Field label="Subscription Plan">
                    <Select value={form.plan} onValueChange={(v) => set("plan", v)}>
                      <SelectTrigger data-testid="school-plan-select"><SelectValue /></SelectTrigger>
              ...
              <TableBody data-testid="schools-table">
                {view.map((s) => (
                  <TableRow key={s.id} data-testid={`school-row-${s.code}`} className="cursor-pointer" onClick={() => navigate(`/app/schools/${s.id}`)}>
                    ...
                    <TableCell className="text-xs">{s.subscription?.plan || "—"}</TableCell>
